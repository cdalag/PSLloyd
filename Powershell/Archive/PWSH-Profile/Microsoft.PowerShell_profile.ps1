oh-my-posh init pwsh --config C:\Users\CDalag\OneDrive\.oh-my-posh.omp.json | Invoke-Expression

function GodMode {explorer "C:\Users\CDalag\OneDrive\GodMode.{ED7BA470-8E54-465E-825C-99712043E01C}"}
...
function Update-PWSH {
	...
	scoop cache rm *
	scoop cleanup *
	Update-Module -Name * -Verbose -Confirm
	Update-PSResource -Name * -Verbose -Confirm
	winget upgrade
}	

function Restart-Explorer {
    CMD /C taskkill /IM explorer.exe /F
    CMD /C DEL "%localappdata%\IconCache.db" /A
...
function ytDLP {
    $DirPath = Read-Host -Prompt "Directory Path"
    if ($DirPath -eq "") {
        $DirPath = "C:\Users\CDalag\OneDrive\Music\YT-DLP"
    }
    ...
    while (-not  (Test-Path -Path $DirPath -PathType Container)) {
        $DirPath = Read-Host -Prompt "Enter a VALID directory path"
        if ($DirPath.StartsWith(" ") -or $DirPath.EndsWith(" ")) {
            $DirPath = $DirPath.Trim(" ")
            $DirPath
        }
    ...
    yt-dlp --paths $DirPath --quiet --progress --newline --no-playlist --embed-thumbnail --convert-thumbnails png --extract-audio --audio-format flac --audio-quality 0 --windows-filenames --force-overwrites $(Read-Host -Prompt "Youtube Link")
}

function Get-NewIcons {
...
"@
    ...
    $outputDir = "C:\Users\CDalag\Desktop\NewIcons\"
    if (-not (Test-Path -Path $outputDir -PathType Container)) {
        New-Item -ItemType Directory -Path $outputDir | Out-Null
    }

    $backupDir = "C:\Users\CDalag\OneDrive\ICONS\"
...
function Get-Icons {
    Get-NewIcons
    # Copy icon items to OneDrive folder
    $outputDir = "C:\Users\CDalag\Desktop\NewIcons\"
    if (-not (Test-Path -Path $outputDir -PathType Container)) {
        New-Item -ItemType Directory -Path $outputDir | Out-Null
    }
    ...
    foreach ($currentItemName in $(Get-ChildItem -Path $outputDir)) {
        $iconIndex = 0
        $backupItemName = $(Get-ChildItem -Path $backupDir -Filter $currentItemName.Name)
        $currentItemHash = Get-FileHash -Path $currentItemName -Algorithm MD5
        $backupItemHash = Get-FileHash -Path $backupItemName -Algorithm MD5
        if ((Test-Path -Path $backupDir -Filter $currentItemName.Name)) {
            Read-Host -Prompt $backupItemName.FullName
            Write-Host -Object ("newIconHash   : " + $currentItemHash.Hash) -ForegroundColor Green
            Write-Host -Object ("backupIconHash: " + $backupItemHash.Hash) -ForegroundColor Yellow
            Read-Host -Prompt "HASHES"
            if (($currentItemHash.Hash -eq $backupItemHash.Hash)) {
                while (Test-Path -Path $backupItemName -PathType Leaf) {
                    Read-Host -Prompt $backupItemName
                    # $newName = Rename-Item -Path $backupItemName -NewName ($backupItemName.BaseName + ".$iconIndex.ico") -Verbose
                    $newName = $backupDir + $backupItemName.BaseName + ".$iconIndex.ico"
                    $backupItemName = New-Item -Path $newName

                    # Read-Host -Prompt "NEWWWWWWWWWWW $newName"
                    # $backupItemName = New-Item -Path $newName
                    Read-Host $newName
                    ++$iconIndex
                }
                Read-Host -Prompt "HASHED"
                Move-Item -Path $currentItemName -Destination $backupDir -Verbose
            }
        } else {
            Move-Item -Path $currentItemName -Destination $backupDir
            Write-Host -Object ("newIconHash   : " + $currentItemHash.Hash) -ForegroundColor Green
            Write-Host -Object ("backupIconHash: " + $backupItemHash.Hash) -ForegroundColor Yellow
        }
    }
